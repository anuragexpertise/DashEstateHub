...
CREATE INDEX idx_users_society_role
ON users(society_id, role);

CREATE INDEX idx_users_linked
ON users(linked_id);
CREATE INDEX idx_apartments_society
ON apartments(society_id);

CREATE INDEX idx_apartments_active
ON apartments(society_id, active);

CREATE INDEX idx_apartments_flat
ON apartments(society_id, flat_number);
CREATE INDEX idx_vendors_society_active
ON vendors(society_id, active);

CREATE INDEX idx_vendors_service
ON vendors(service_type);

CREATE INDEX idx_security_society_active
ON security_staff(society_id, active);

CREATE INDEX idx_trx_society_date
ON transactions(society_id, trx_date);

CREATE INDEX idx_trx_account
ON transactions(acc_id);

CREATE INDEX idx_trx_entity
ON transactions(entity_id);

CREATE INDEX idx_trx_status
ON transactions(status);

CREATE INDEX idx_trx_society_status_date
ON transactions(society_id, status, trx_date);

CREATE INDEX idx_gate_society_time
ON gate_access(society_id, time_in);

CREATE INDEX idx_gate_entity
ON gate_access(role, entity_id);

CREATE INDEX idx_gate_open_entries
ON gate_access(role, entity_id, time_out);
CREATE INDEX idx_attendance_security
ON attendance(security_id);

CREATE INDEX idx_attendance_date
ON attendance(society_id, time_in);

CREATE INDEX idx_apt_charges_society
ON apt_charges_fines(society_id, apt_id);

CREATE INDEX idx_apt_charges_date
ON apt_charges_fines(start_date, end_date);
CREATE INDEX idx_ven_charges_society
ON ven_charges_fines(society_id, ven_id);

CREATE INDEX idx_ven_charges_date
ON ven_charges_fines(start_date, end_date);

CREATE INDEX idx_sec_charges_society
ON security_charges_fines(society_id, sec_id);

CREATE INDEX idx_accounts_society
ON accounts(society_id);

CREATE INDEX idx_accounts_parent
ON accounts(parent_account_id);

CREATE INDEX idx_accounts_tab
ON accounts(tab_name);
CREATE INDEX idx_asset_society
ON asset_register(society_id);

CREATE INDEX idx_asset_account
ON asset_register(parent_account_id);

CREATE INDEX idx_trx_paid_only
ON transactions(society_id, trx_date)
WHERE status='paid';
